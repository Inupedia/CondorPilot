"""Test one defined-risk Bull Put management hypothesis: remove the option-price stop.

Entry remains the prior-day-close-above-SMA200 rule. The spread remains defined risk through its
5-point long-put wing. The 50% take-profit and 21-DTE time exit remain unchanged. The only rule
removed is the 2x-credit option-price stop; there is no stop-multiple grid and no replacement
threshold.

This hypothesis follows diagnostics performed on all SPY periods, so the three SPY ranges are
model-development robustness checks, not untouched OOS evidence. QQQ remains untouched unless
the same frozen no-stop rule is positive in every SPY range.
"""
...
def _profit_factor(trades: list[NoStopTrade]) -> float | None:
...
def _no_stop_exit(
    *,
    ...
    """Return an exit reason while deliberately omitting any price-stop condition."""
    if entry_credit <= 0:
    ...
    if profit >= entry_credit * profit_target_fraction:
        return "take_profit"
    if dte <= exit_dte:
        return "time_exit"
    return None
...
def _run_period(
    ...
    *,
    ...
    if position is not None:
        raise RuntimeError(f"no-stop period {start} through {end} ended with an open position")
    ...
    return {
        ...
        "best_trade": max((item.net_pnl for item in trades), default=0.0),
...
def main() -> int:
    ...
    for label, start, end, minimum_pf in definitions:
        ...
        result["net_pnl_change_vs_price_stop_baseline"] = (
    ...
    output = {
        "method": {
            ...
            "new_management_rule": "no option-price stop; defined-risk wing remains",
            "kept_exits": "50% take profit and 21-DTE time exit",
            "new_tunable_parameters": 0,
            "other_rules": (
                "45 DTE / 15-delta short put / exact 5-point wing / 2% account risk / "
                "existing slippage and commissions"
            ),
            "qqq_policy": "QQQ remains untouched unless the no-stop rule passes every SPY stage",
        },
        "stages": stages,
